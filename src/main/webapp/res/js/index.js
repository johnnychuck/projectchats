var sip = "127.0.0.1:8080/mim"
var ws;
$(function() {
    ws = new WebSocket('ws://'+sip+'/wsim.do?id='+aid);
    ws.onmessage = function(evnt) {receivemsg(evnt.data) };
    ws.onopen  = function(evnt){ console.info("服务器链接成功..."); };
    ws.onclose = function(evnt){ console.info("服务器链接断开..."); };
    $("#send").click(function() {
		var msg = $("#msgarea").val();
		var nowtime = getNowFormatDate();
		var message = '<h4><small><a href="">'+aname+'</a> '+nowtime+' 发送：</small></h4><h5>'+msg+'</h5>';
		var smsg= '{"mid":"0","fromtype":"user","fromid":"'+aid+'","fromname":"'+aname+'","msgtime":"'+nowtime+
			'","targetid":"10000","targetname":"聊天测试组","targettype":"2","msgtext":"'+msg+'","msgextras":"","msgtype":"text"}';
   		ws.send(smsg);
		$(".well").append(message);
...
function receivemsg(msgdata) {
	...
	var message = '<h4><small><a href="">'+msg.fromname+'</a> '+msg.msgtime+' 发送：</small></h4><h5>'+msg.msgtext+'</h5>';
	$(".well").append(message);
	$(".well").scrollTop( $(".well")[0].scrollHeight );
	$("#msgarea").val("");
}
